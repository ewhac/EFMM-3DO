...
/* The following constant is the maximum number of chunks that can be retained
 * by subscribers at any one time. A message is sent for each chunk in the stream
 * to its subscriber. When the subscriber is finished with the data, it replys
 * to the message to signal that its finished with the chunk, allowing the
 * buffer to eventually be recycled. If we run out of messages, we can't deliver
 * data (arrgh!).
 */
#define	DS_MAX_SUBS_MESSAGES	( 128 + DS_MAX_SUBSCRIBERS )
...
typedef enum DSErrorCode {
		...
		kDSBadBufAlignErr,			/* buffer list passed to DSHOpenStream contains */
...
/* valus for options in DataAcqMsg.msg.marker.options */

#define	GOMARKER_ABSOLUTE		0				/* value == absolute marker position */
#define	GOMARHER_FORWARD		1				/* value == count of markers to skip forward */
#define	GOMARKER_BACKWARD		2				/* value == count of markers to skip backward */
