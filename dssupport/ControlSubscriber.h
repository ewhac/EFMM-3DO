 ...
 *						because we need to allocate msg buffer space as well as message items.
 *						Change dsReqMsgItemPool to dsReqMsgPool.
 ...
 *						Add CTRL_NUM_DS_REQS_MSGITEMS.
 *	8/5/93		jb		Add CTRL_MAX_SUBS_MESSAGES
 *						Added subsReplyPort, subsReplyPortSignal, and subsMsgPool
 *						to the context block.
 *	6/28/93		jb		Add ALRM, PAUS, and STOP subchunk types
 *	6/15/93		jb		Change branch chunk type from 'MRKR' to 'GOTO'
 *						Remove include of "graphics.h", replace with "types.h"
 *						Switch to using SubsChannel instead of custom channel struct
 *						that is identical with the generic version.
